...
local shapeshifter = require("shapeshifter")
...
function sampleNPC.onInitAPI()
	...
	-- registerEvent(sampleNPC, 'onNPCCollect')
end
...
function sampleNPC.onTickEndNPC(v)
	...
	if v.despawnTimer <= 0 then
		...
		-- if data.npc and data.npc.isValid then data.npc:kill(9) end
	...
	if not data.initialized then
		...
		local npc = NPC.spawn(v.ai1, v.x, v.y + v.height, v.section, true)
		npc.direction = v.direction
		npc.y = npc.y - npc.height
		npc.dontMove = v.dontMove
		npc.despawnTimer = v.despawnTimer

		data.npc = npc
		data.name = settings.name

		npc.data.parent_name = data.name
		npc.data.parent = v
	...
	if not npc.isValid then return v:kill(9) end
