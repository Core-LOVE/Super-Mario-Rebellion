...
local size = vector(800, 600)
local shader = Shader()
shader:compileFromFile(nil, "scripts/shaders/retroResolution.frag")

function retroResolution.onInitAPI()
    registerEvent(retroResolution, "onCameraUpdate")
	registerEvent(retroResolution, "onDraw")
	registerEvent(retroResolution, "onFramebufferResize")
end
...
function retroResolution.onFramebufferResize(width, height)
