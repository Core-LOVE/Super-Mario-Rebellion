...
do
    local function setVoice(box,name)
        ...
        if name == nil or name == "" then
            ...
            local path = Misc.resolveSoundFile("scripts/littleDialogue/portraits/".. name) or Misc.resolveSoundFile("scripts/littleDialogue/".. name) or Misc.resolveSoundFile(name)
    ...
    local function loadImage(settings,styleName,name,imageFilename)
        local path = Misc.resolveFile("scripts/littleDialogue/".. styleName.. "/".. imageFilename) or Misc.resolveFile("scripts/littleDialogue/".. imageFilename)
    ...
    local function loadSound(settings,styleName,name,soundFilename)
        settings[name] = Misc.resolveSoundFile("scripts/littleDialogue/".. styleName.. "/".. soundFilename) or Misc.resolveSoundFile("scripts/littleDialogue/".. soundFilename)
    end

    local function findFont(styleName,fontFileName)
        local folderPath = "scripts/littleDialogue/".. styleName.. "/".. fontFileName
        if Misc.resolveFile(folderPath) ~= nil then
            return folderPath
        end

        local mainPath = "scripts/littleDialogue/".. fontFileName
    ...
    local function compileShader(styleName,vertName,fragName)
        local vertPath = Misc.resolveFile("scripts/littleDialogue/".. styleName.. "/".. vertName) or Misc.resolveFile("scripts/littleDialogue/".. vertName)
        local fragPath = Misc.resolveFile("scripts/littleDialogue/".. styleName.. "/".. fragName) or Misc.resolveFile("scripts/littleDialogue/".. fragName)
    ...
    function littleDialogue.registerStyle(name,settings)
        ...
        settings.typewriterSound = SFX.open(Misc.resolveSoundFile("scripts/littleDialogue/".. name.. "/typewriter") or Misc.resolveSoundFile("scripts/littleDialogue/typewriter"))

        settings.font = settings.font or textplus.loadFont("graphics/font.ini")
        settings.easing = settings.easing or easing.linear
    ...
    local function getKeyImageFilename(imageName)
        if currentlyUpdatingBox ~= nil then
            ...
            local stylePath = "scripts/littleDialogue/".. currentlyUpdatingBox.styleName.. "/keys/".. imageName.. ".png"
        ...
        local globalPath = "scripts/littleDialogue/keys/".. imageName.. ".png"
    ...
    function littleDialogue.getPortraitData(name)
        if portraitData[name] == nil then
            local txtPath = Misc.resolveFile("scripts/littleDialogue/portraits/".. name.. ".txt")
            local data
            ...
            local imagePath = Misc.resolveGraphicsFile("scripts/littleDialogue/portraits/".. name.. ".png")
...
function boxInstanceFunctions:draw()
    ...
    local ease = easing.outCubic

    if self.state == STATE.OUT then
        ease = easing.inCubic
    end 

    local scaleX = ease(self.openingProgress, self.settings.openStartScaleX, 1, 1)
    local scaleY = ease(self.openingProgress, self.settings.openStartScaleY, 1, 1)
    local opacity = ease(self.openingProgress, self.settings.openStartOpacity, 1, 1)
    ...
    if self.settings.windowingOpeningEffectEnabled and self.openingProgress < 1 then
        ...
        scaleX = ease(blackProgress, self.settings.openStartScaleX, 1, 1)
        scaleY = ease(blackProgress, self.settings.openStartScaleY, 1, 1)
        opacity = ease(blackProgress, self.settings.openStartOpacity, 1, 1)

        boxCutoffWidth = ease(mainProgress, self.settings.openStartScaleX, 1, 1) * self.totalWidth
        boxCutoffHeight = ease(mainProgress, self.settings.openStartScaleX, 1, 1) * self.totalWidth
